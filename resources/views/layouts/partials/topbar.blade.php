<section class="top">
    <div class="topbar">
        ...
        <div class="topbar-right" style="display: flex; align-items: center; gap: 10px;">
            <button data-bs-toggle="modal" data-bs-target="#paymentModal" class="btn btn-primary rounded-pill" style="padding: 10px 20px; font-size: 1.2rem; font-weight: bold; text-decoration: none;">
            ...
            <a href="{{ route('settings') }}" class="btn btn-primary rounded-circle" style="width: 50px; height: 50px; padding: 0; display: flex; align-items: center; justify-content: center; overflow: hidden;" title="Cài đặt">
                @if (auth()->user()->avatar_image_url)
                    <img src="{{ auth()->user()->avatar_image_url }}" style="width: 40px; height: 40px; object-fit: cover;" alt="Avatar của {{ auth()->user()->name }}" class="rounded-circle">
                @else
...
@include('layouts.partials.payment')
