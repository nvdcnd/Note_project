...
it('renders every queued mailable through the worker path: serialize, unserialize, render', function () {
    ...
    $queued = [
        ...
        // Token null: có token, template sẽ render route('invitation.show') đang
        // bị comment khỏi routes — nhánh đó nằm ở test skip bên dưới.
        new Mail40account('chuadangky@example.com', $note, null),
...
it('renders the invitation mails that carry an invite token', function () {
    $sender = User::factory()->create();
    $note = Note::factory()->create(['creater_id' => $sender->id]);
    $organization = Organization::factory()->create(['hostID' => $sender->id]);
    $token = str_repeat('a', 64);
