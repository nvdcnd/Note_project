# Organization balance & transaction view
...
Trang này là trang hiện những lịch sử giao dịch và hỗ trợ chuyển point của organization
## 1. Overview
### 1.1. Top bar
- Kế thừa từ file "../org/org_list_view.md"
### 1.2. Balance Hero
- Hero này chứa thông tin về số point của organization
#### 1.2.1. Content
- "Xin chào <tên organization>, hiện bạn đang có" (p)
- Xuống dòng và ghi số point bằng h1
### 1.3. Button group
#### 1.3.1. Nút nạp tiền
- Nằm bên trái của màn hình (Desktop & Tablet) hoặc xếp trên (Mobile)
- Bấm vào sẽ gọi pop-up nạp point riêng của organization
#### 1.3.2. Nút chuyển tiền
- Nằm bên phải của màn hình (Desktop & Tablet) hoặc xếp dưới (Mobile)
- Bấm vào sẽ gọi pop-up chuyển point riêng của organization
#### 1.3.4. Pop-up nạp point của Organization
...
#### 1.3.5. Pop-up chuyển point của Organization
- Sử dụng pop-up của bootstrap
#### 1.3.5.1. Header
- Chỉ có nút đóng (chữ "X") tại góc trên bên phải của card pop-up
#### 1.3.5.2. Body
- Title "Organization chuyển point" căn giữa
...
- Input To (đến):
    - Nếu chuyển cho user: Nhận email
    - Nếu chuyển cho organization khác: Nhận ID
...
#### 1.4.1. Header
- Chứa "From. ORG #<ID của organization này> - to <email user/ID organization>"
#### 1.4.2. Body
